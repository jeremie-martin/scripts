...
TEXT_EXTS = {
    "txt", "log", "cfg", "conf", "ini", "toml", "yaml", "yml", "json", "jsonc",
    "csv", "tsv", "ndjson", "ipynb", "md", "markdown", "rst", "adoc", "tex",
    "bib", "html", "htm", "xhtml", "shtml", "xml", "xsd", "xsl", "xslt", "dtd",
    "svg", "css", "scss", "sass", "less", "sh", "bash", "zsh", "fish", "ps1",
    "psm1", "bat", "cmd", "make", "mk", "cmake", "gradle", "groovy", "c", "h",
    "cpp", "cc", "cxx", "hpp", "hh", "hxx", "m", "mm", "cs", "java", "kt",
    "kts", "go", "rs", "py", "rb", "php", "pl", "pm", "r", "jl", "swift",
    "scala", "clj", "cljs", "edn", "hs", "lhs", "ex", "exs", "erl", "lua",
    "ts", "tsx", "js", "jsx", "mjs", "cjs", "coffee", "sql", "dbml", "proto",
    "idl", "nix", "sol", "mdx", "dockerfile", "env", "tf", "tfvars", "nomad", "hcl",
}
...
class Config:
    ...
    def merge(self, other: "Config") -> "Config":
        ...
        merged.use_default_excludes = other.use_default_excludes if other.use_default_excludes != default.use_default_excludes else self.use_default_excludes
...
def load_toml_config(path: Path) -> Optional[Config]:
...
def find_global_config() -> Optional[Config]:
...
def find_project_config(start_dir: Path = None) -> Optional[Config]:
...
def format_tree_output(files: List[str], config: Config):
    ...
    if len(files) > 0:
        ...
        sorted_files = sorted(
            [(fp, file_stats[fp]["lines"], file_stats[fp]["chars"]) 
             for fp in files],
            key=lambda x: x[2],
            reverse=True
        )[:10]
